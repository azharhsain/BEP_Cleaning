...
*Purpose: Baseline market business survey data checking outliers and dropping or changing observations 
...
* INCLUDING MACROS AND I required pkg
...
* CLEANING OUTLIERS
	...
	****Section A, E & F****
	...
	foreach var in `outlier_var' {
		cap histogram `var', freq
		cap graph export "`var'.png", replace
	}
	
	****Section B****
	foreach i1 of numlist 1/28 {
		cap histogram b1_tot_wages`i1', freq
		cap graph export "b1_tot_wages`i1'.png", replace
	...
	foreach i2 of numlist 1/10 {
		cap histogram b2_tot_wages`i2', freq
		cap graph export "b2_tot_wages`i2'.png", replace
	...
	foreach i3 of numlist 1/2 {
		cap histogram b3_tot_wages`i3', freq
		cap graph export "b3_tot_wages`i3'.png", replace
	...
	foreach i4 of numlist 1/1 {
		cap histogram b4_tot_wages`i4', freq
		cap graph export "b4_tot_wages`i4'.png", replace
	...
	foreach i5 of numlist 1/12 {
		cap histogram b5_tot_wages`i5', freq
		cap graph export "b5_tot_wages`i5'.png", replace
	...
	foreach i6 of numlist 1/2 {
		cap histogram b6_tot_wages`i6', freq
		cap graph export "b6_tot_wages`i6'.png", replace
	...
	****Section C****
	...
	desktop drill elecgrill elecmill elecsaw electrictool elecweight emerlight fan genset gascooker ///
	...
	tractor van almirah bed bench chair counter desk dresstable shelve showcase stool woodbox woodshop
	...
	* 3. Generate scatter plots for each asset (average resale value vs. number owned) for each business type
	...
	foreach x of local assets{
		...
		//calculate single variables documening the number anf resale value of each asset (consolidating variable by business type)
		...
		//calculate average resale value of each item
		cap gen c_`x'_AV=c_`x'_sale/c_`x'_own
		cap egen nmiss=rownonmiss(c_`x'_AV)
	
		//generate log of average resale value
		cap gen ln_c_`x'_AV=log(c_`x'_AV) if sum(nmiss)>0
	
		
		//check distribution of log of average resale value, only if at least one business had any of the asset
		dotplot ln_c_`x'_AV if sum(nmiss)>0
		capture graph export ln_c_`x'_AV.eps, replace
		
		//scatter plots of average resale value by number owned for each business type
		scatter c_`x'_AV c_`x'_own, by(_retail_type) ylabel(,labsize(vsmall)) saving(`x'_scatter_retail, asis replace)
		
		scatter c_`x'_AV c_`x'_own, by(_workshop_type) ylabel(,labsize(vsmall)) saving(`x'_scatter_workshop, asis replace)

		
		scatter c_`x'_AV c_`x'_own, by(_service_type) ylabel(,labsize(vsmall)) saving(`x'_scatter_service, asis replace)
	...
	*Outliers coded in same directory, file BIZ-MARKET_C_OUTLIERS.xlsx
	...
	foreach x of local assets{
		...
		// set individual asset by business type to missing if the rowtotal values are missing
...
//SET APPROPRIATE OUTLIERS TO MISSING. Refer to D_MKT_BIZ_BOXPLOTS.pdf and qplots-D-MKT-BIZ.pdf (lognormal distribution check) in output.
	...
	foreach i of numlist 1/7 {
		capture confirm variable d`i'_elec_item_val
		
		if _rc!=0 {
			display "no variable"
		} 
		else {
			replace d`i'_elec_item_val=.y if d`i'_elec_item_val==0
			replace d`i'_elec_item_val=.z if d`i'_elec_item_val>=4000000
		}
	}
