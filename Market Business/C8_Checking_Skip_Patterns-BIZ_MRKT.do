...
Purpose: Baseline market business survey variables' skip pattern checking
...
* CHECKING SKIP PATTERNS
	...
	assert (a5_rent!=.) if (a4_owner==2) !=0
...
* FORCING CHANGES
	...
	//// Section - C [Information about physical assets owned by the business]
	// Surveoyor has entered 0 in number of lorry, van, autorickshaw, woodbox and shelves count, so assigning missing values to all related variables
	replace c8_autorickshaw_have=2 if c8_autorickshaw_own==0
	replace c8_autorickshaw_sale=. if c8_autorickshaw_own==0	
	replace c8_autorickshaw_own=. if c8_autorickshaw_own==0	
	replace c8_lorry_have=2 if c8_lorry_own==0
	replace c8_lorry_sale=. if c8_lorry_own==0	
	replace c8_lorry_own=. if c8_lorry_own==0	
	replace c8_van_have=2 if c8_van_own==0
	replace c8_van_sale=. if c8_van_own==0	
	replace c8_van_own=. if c8_van_own==0
	replace c9_shelve_have=2 if c9_shelve_own==0
	replace c9_shelve_sale=. if c9_shelve_own==0	
	replace c9_shelve_own=. if c9_shelve_own==0	
	replace c9_woodbox_sale=. if c9_woodbox_own==0	
	replace c9_woodbox_own=. if c9_woodbox_own==0	
	...
	// For all those entries in which shared genset expenditure is 0, unit of expenditure should be missing
	replace e12_genset_shared_exp_unit=. if e12_genset_shared_exp_amount==0
	
	//// Section - F [Information about electricity supply situation]
	
	//// Section - G [Information about expenditure, revenue and credit of the business]
	// Surveyor has marked yes in question asking for income generation from property rent & bank account interest, but entered 0 in amount field
	replace g26_income_rent_int=2 if g26_income_rent_int_amount==0
	replace g26_income_rent_int_amount=. if g26_income_rent_int_amount==0
